...
function Catalog(){
 ...
 <section className="catalogGrid">{items.map(item=><article className="propertyCard" key={item.id}>{item.imageUrl&&<div className="propertyImage" style={{backgroundImage:`url(${item.imageUrl})`}}><span>{preview?c.illustrative:c.published}</span></div>}<div className="cardTop"><div><div className="eyebrow">{item.project}</div><h2>{item.unit??item.headline??c.publishedOnly}</h2></div><label className="select"><input type="checkbox" checked={selected.includes(item.id)} onChange={()=>toggle(item.id)} disabled={!selected.includes(item.id)&&selected.length>=4}/> {c.select}</label></div><p>{item.summary??c.detailIntro}</p><div className="cardMetrics"><span><small>{c.score}</small><b>{item.analytics.riskAdjustedScore?.toFixed?.(1)??"—"}</b></span><span><small>{c.netYield}</small><b>{item.analytics.netYield==null?"—":`${(item.analytics.netYield*100).toFixed(2)}%`}</b></span><span><small>{item.analytics.productionReturnMetric??c.return}</small><b>{item.analytics.productionReturn==null?"—":`${(item.analytics.productionReturn*100).toFixed(2)}%`}</b></span></div><a className="detailLink" href={pageHref(`id=${item.id}`)}>{c.openAnalysis}</a></article>)}</section></>}
 </main><footer>PrivatePhuket · {c.platform} · {c.analyticsDisclaimer}</footer></>;
}
...
function PropertyDetail({id}:{id:string}){
 ...
 <div className="metric"><span className="muted">{c.netYield.toUpperCase()}</span><b>{a.netYield==null?"—":`${(a.netYield*100).toFixed(2)}%`}</b></div>
