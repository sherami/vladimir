...
function PropertyDetail({id}:{id:string}){
 ...
 <div className="disclosure">{c.runPrefix} <b>{a.calculationRunId}</b>. {c.engine} {a.engineVersion}; {c.methodology} {a.methodologyVersion}; {c.snapshot} {String(a.inputSnapshotHash).slice(0,16)}… {preview?c.previewProvenanceTail:c.provenanceTail}</div></section>
 </main><footer>PrivatePhuket · {c.platform} · {c.analyticsDisclaimer}</footer></>
}
function App(){
